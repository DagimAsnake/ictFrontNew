...
function ChangePasswordInvestor() {
  const navigate = useNavigate();

  const { invId, token } = useParams();
  ...
  const submitHandler = (e) => {
    ...
    const loginRequest = async () => {
      const response = await fetch(
        `http://localhost:8080/auth/investor/passwordreset/${invId}/${token}`,
        {
          method: "POST",
          body: JSON.stringify({
...
export default ChangePasswordInvestor;
